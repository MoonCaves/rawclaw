...
func TestAll_SourceFilterAppliesToArchiveScopes(t *testing.T) {
	...
	if n := foreignCount(All(t.Context(), "claude", false, false)); n != 1 {
		t.Errorf("--source claude foreign scopes = %d, want 1", n)
	}
	if n := foreignCount(All(t.Context(), "codex", false, false)); n != 0 {
...
func TestOrphanScanSkipsArchiveDBs(t *testing.T) {
	archivetest.Setup(t, "")
	_ = All(t.Context(), "", false, false) // the splice creates archive-*.db files in the cache dir
