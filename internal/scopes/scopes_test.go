...
func (scopedContainerSource) Messages(source.Container) ([]model.Message, error) { return nil, nil }
...
func TestContainerScopesFiltersCWDsBeforeIndexing(t *testing.T) {
	...
	got := containerScopes("test", adapter, func(cwd string) string { return cwd }, false,
		func(cwd string) bool { return cwd == matching })
	if len(got) != 1 || got[0].CWD != matching {
		t.Fatalf("containerScopes returned %+v, want only %q", got, matching)
	}
