...
// TestTagWriteDefaultScopeConsolidatedOnlyDoesNotBlock proves the remaining
// nil-scope deadlock: a retained session with no source/catalog row resolves
// to consolidated.db, then waits behind the fence that a concurrent rebuild
// already holds. The command must author the tag without waiting for derived
// publication.
