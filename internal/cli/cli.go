...
func runSearch(ctx context.Context, w io.Writer, o *Options, args []string) error {
	...
	// An explicit --reindex or --dir refreshes the targeted project. For
	// --this-project, the consolidated store's O(1) freshness check skips the
	// four-runtime discovery/reconsolidation pass when the project is unchanged.
	// Default search is answer-first: stale or UNKNOWN answers from the
	// consolidated store are reported while a throttled background ingest nudge
	// repairs freshness.
