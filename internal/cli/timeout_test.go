...
func TestResolveTimeoutFromArgsConsolidateFloor(t *testing.T) {
	...
	tests := []struct {
		...
		{"ingest with short flag -d gets the floor", []string{"-d", "/tmp", "ingest"}, "", consolidateWatchdog},
		{"ingest with short flag -s gets the floor", []string{"-s", "claude", "ingest"}, "", consolidateWatchdog},
		{"ingest with empty string session arg gets floor", []string{"ingest", ""}, "", consolidateWatchdog},
