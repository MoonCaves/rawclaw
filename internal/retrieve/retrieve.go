...
func Explain(covs []int, in ExplainInputs) []ScoreExplain {
	...
	for i, cov := range covs {
		tier := "normal"
		if i < len(in.Routines) && in.Routines[i] {
			tier = "routine"
		}
		e := ScoreExplain{
			Coverage: cov,
...
func SearchExplained(dbp, q string, limit int, p SearchParams) (out []Hit, explains []ScoreExplain) {
	...
	out = make([]Hit, 0, n)
	covs := make([]int, 0, n)
	routines := make([]bool, 0, n)
	for i := 0; i < n; i++ {
		out = append(out, scored[i].Hit)
		covs = append(covs, scored[i].cov)
		routines = append(routines, scored[i].Routine)
	}
...
func searchScored(dbp, q string, limit int, p SearchParams) ([]scoredHit, ExplainInputs) {
	...
	if p.Exact {
		...
		// policy: 2026-09-07 single stemmed query; messages_fts (porter) is a measured superset of messages_fts_exact, 0 exact-only rows on 4 queries see docs/design/exact-tier-notes.md
