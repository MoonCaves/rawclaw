...
func TestRefreshDBPath_RetainsInUseStaleSQLite(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("XDG_CACHE_HOME", filepath.Join(home, ".cache"))
	...
	stale := filepath.Join(dir, "in-use.db")
	holder, err := store.ConnectRW(stale)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(stale); err != nil {
		holder.Close()
		t.Fatalf("in-use stale refresh db was removed: %v", err)
	}

	if _, err := holder.Exec("ROLLBACK"); err != nil {
		holder.Close()
		t.Fatal(err)
	}
