...
// TestWarningsTellTheTruthInTheirOneStoreShape covers the completeness report in
// its one-store shapes. A JSON envelope carrying an honest not_consolidated row
// is worth nothing if the advisories stay silent about it, and the same goes for
// a search that quietly fell back to the fan-out: both are degradations, and
// both have to reach the caller as warnings rather than as prose the renderer
// invents (#169).
func TestWarningsTellTheTruthInTheirOneStoreShape(t *testing.T) {
	full := buildWarnings(warningInputs{
		reports: []ScopeReport{{Dir: "p.db", Status: ScopeSearched, Detail: "12 sessions"}},
	})
	if hasCode(full, WarnNotConsolidated) || hasCode(full, WarnStoreFallback) {
		t.Errorf("a complete one-store answer raised a degradation warning: %+v", full)
	}

	partial := buildWarnings(warningInputs{
		reports: []ScopeReport{
	...
	w := findWarning(partial, WarnNotConsolidated)
	if w == nil {
		t.Fatalf("warnings hide the unfolded project databases: %+v", partial)
	}
	if got := w.Facts["databases"]; got != 2 {
		t.Errorf("databases fact = %v, want 2", got)
	}
	if !strings.Contains(w.Message, "2 project database(s) are not in the one store") {
		t.Errorf("message hides the unfolded project databases:\n%s", w.Message)
	}
	if !strings.Contains(w.Message, "rawclaw consolidate") {
		t.Errorf("message names the problem without the fix:\n%s", w.Message)
	}

	fell := buildWarnings(warningInputs{
		reports:   []ScopeReport{{Dir: "p.db", Status: ScopeSearched}},
		storeNote: "one store unavailable — searched per project instead",
	})
	f := findWarning(fell, WarnStoreFallback)
	if f == nil {
		t.Fatalf("warnings hide the fallback to the fan-out: %+v", fell)
	}
	if !strings.Contains(f.Message, "one store unavailable") {
		t.Errorf("fallback warning drops the reason:\n%s", f.Message)
	}
	if got := f.Facts["store"]; got != StorePerProject {
		t.Errorf("store fact = %v, want %q", got, StorePerProject)
	}
}
