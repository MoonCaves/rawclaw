...
- **Durable session catalog and catalog-first O(1) session resolution.** `rawclaw setup` now installs
  ...
  `$XDG_DATA_HOME/rawclaw/catalog`). Session resolution (`read`, `outline`, `tag-prep`, `--resume`)
  queries the catalog first for instant O(1) resolution without scanning project directories on
  disk, with stem resolution for subagents and session ID prefixes.
- **Proactive background session ingest via `rawclaw ingest`.** `rawclaw setup` hooks now trigger
...
- **Routine verdict tagging and search partition.** `rawclaw tag-write --routine` records a `routine`
  ...
  engineering discussions to surface first while keeping routine sessions fully searchable.
  `--debug-search` displays the active ranking tier (`normal` vs `routine`).
- **Vector coverage reporting and partial-embedding warnings.** When semantic search runs with an
  embedder, RawClaw measures candidate message coverage across the index. If candidate messages lack
  vector embeddings, search results and `--json` emit a structured `vector_gap` warning stating the
  exact missing count and directing the user to run `rawclaw --reindex-vectors`.
...
- **Consolidated single-connection browse.** Multi-project and scoped browse (bare `rawclaw`,
  ...
  of opening dozens of per-project database files sequentially. Ordering includes a deterministic
  session ID tiebreaker.
- **SQLite read-path tuning with 256 MiB memory-mapped I/O.** Read-only SQLite database connections
  configure a 256 MiB `mmap_size` window, serving hot database pages directly from memory-mapped
  I/O to accelerate search, read, and browse operations.
- **POSIX single-quoting for `--resume` commands.** Resume commands generated by `--resume`
  (`cd '<dir>' && <cmd>`) wrap directory paths in POSIX single quotes instead of double quotes,
  preventing accidental shell variable expansions (`$HOME`, `$VAR`) or command substitutions when
  pasted into a terminal.

### Fixed

- **Semantic index worker goroutines no longer leak on database write errors.** `VecIndex` properly
  terminates background worker goroutines if a database upsert fails mid-batch, preventing goroutine
  leaks during partial vector indexing failures.
- **Antigravity transcript parsing fidelity and scanner buffer limits.** The Antigravity adapter
  increases its line scanner buffer to 1 MB to prevent scan errors on oversized transcript records,
  prioritizes tool-call `Cwd` over configuration headers to ensure accurate workspace detection, and
  resolves subagent lineage directly from transcript records rather than history membership. Backing
  file switches (such as transitioning from `transcript.jsonl` to `transcript_full.jsonl`) cleanly
  drop stale watermarks to prevent retained sessions from falsely disappearing.
- **Goose adapter SQL identifier sanitization and error handling.** Dynamic table and column
  identifiers discovered from SQLite schemas are validated against an alphanumeric allowlist to guard
  against SQL injection on untrusted schemas. Row iteration errors (`rows.Err()`) are explicitly
  checked to prevent silent message truncation, and databases moved without SQLite WAL sidecars
  degrade cleanly to last-checkpointed state.
- **Tombstone pruning and multi-source tracking during consolidation.** Store consolidation prunes
  tombstones unconditionally so deleted sessions do not leave orphaned tombstone rows. Multi-source
  merged sessions track individual source databases (`session_sources`), updating cleanly when a
  contributing source file is purged.
- **Scope freshness honesty under lock contention.** When SQLite write locks are contended during
  background indexing passes, affected scopes are marked as stale so search results honestly report
  potential incompleteness rather than falsely claiming freshness.
- **Honest error propagation in topic segmentation checks.** `SessionHasRealSegments` propagates
  underlying SQLite query errors instead of silently swallowing errors as "no real segments".
...
- **`rawclaw consolidate --from-transcripts` rebuilds the store from those transcripts alone.**
  ...
  It refuses to run when the vault holds fewer sessions than the store it would replace — on a
  machine that indexed history before the vault existed, the vault fills as sessions are
  re-indexed, and recovering less than you had is not recovery. Set `RAWCLAW_REBUILD_FORCE=1`
  when the store is known to be junk and you want it gone anyway.
...
- **`archive init` help and the README now steer you to a remote.** `rawclaw archive init --help`
  and the README explain the choice a first-time user actually faces: a bare repo over SSH on a box
  you control is the most private option (transcripts never leave your network, nothing to encrypt);
  a private GitHub/GitLab/Gitea repo is convenient but stores transcripts **unencrypted**, so it
  must stay private. Both also note that adding a second machine is the same `init` command, and
  that machines sharing a hostname need a distinct `--name`. Docs only — no behavior change.
