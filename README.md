...
**Choosing a remote.** Any private git remote works — RawClaw runs no server of its own.
- **Most private:** a bare repo over SSH on a box you control — `git init --bare ~/archive.git` on your own server, then `rawclaw archive init ssh://you@host/~/archive.git` (or `host:archive.git`). Transcripts never leave your network and there's nothing to encrypt.
- **Convenient:** a private GitHub / GitLab / Gitea repo. Transcripts are stored **unencrypted** in the repo, so it must stay private.

**Adding a second machine** is the same command — `rawclaw archive init <same-remote>` clones what the first machine already pushed, so its sessions are searchable immediately. Give machines that share a hostname a distinct `--name`.
