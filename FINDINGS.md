...
Fingerprint comparison against the ledger: `PA-SQLITE-BEGIN-IMMEDIATE-001` (`7e0c7cf3…`) is distinct from `PA-SQLITE-ATOMIC-COMMIT-001` (`c063e5ec…`) because it governs writer admission before snapshot reads, not crash-durable commit; `PA-FTS5-DELETEMERGE-001` (`21ae4bb8…`) is distinct from `PA-CONSOLIDATED-SIDECAR-PRUNE-001` (`d07f69f8…`) because it budgets FTS tombstone compaction, not source-scoped sidecar eligibility; `PA-GO-SINGLEFLIGHT-FALLBACK-001` (`1532e53c…`) is distinct from `PA-K8S-SCOPED-RECONCILIATION-001` (`fe219500…`), `PA-SQLITE-UPSERT-COMPOSITE-001` (`e469d659…`), and `PA-ETCD-CAS-CURSOR-001` (`0da2f5b9…`) because it coalesces in-process duplicate triggers and supplies no durable ownership, identity, or cursor CAS.
