...
## internal/lifecycle
internal/lifecycle/lifecycle.go:L221-247: stdlib: LoadTombstones scanner loop with manual buffering. Restored to 43cb92f scanner behavior per merge-gate ruling (bounded memory, line-over-1MiB rejected, original error wording).
...
## Accepted Deviations
- internal/scopes/container.go:L75: Debug logging on container adapter current-CWD refresh failures (`slog.Debug("scopes: "+sourceID+" current-cwd refresh failed", "cwd", cwd, "err", ierr)`). In 43cb92f, errors in `RefreshAntigravityCWD`/`RefreshGooseCWD` were silently ignored (`_, _, _ = ...`). The unified `refreshContainerCWD` helper logs failures at debug level to assist observability while keeping execution non-fatal. Ruled ACCEPT deliberately.
